...
// Relay drains the deposit module's transactional outbox
// (deposit_outbox) and publishes pending events to JetStream. Delivery is
// at-least-once: an event is only marked published after JetStream
// acknowledges it, and redeliveries are deduplicated by the stream's
// duplicate window keyed on Nats-Msg-Id (the domain event ID). Multiple
// relay instances are safe: MarkPublished is guarded by `published_at IS
// NULL`, so duplicates are bounded and deduplicated anyway.
type Relay struct {
	outboxRepository ports.DepositOutboxRepository
...
func (r *Relay) run(ctx context.Context) {
	...
	r.logger.Info().
		...
		Msg("deposit outbox relay started")

	for {
		select {
		case <-ctx.Done():
			r.logger.Info().Msg("deposit outbox relay stopped: context cancelled")
			return
		case <-r.done:
			r.logger.Info().Msg("deposit outbox relay stopped")
			return
		case <-ticker.C:
			r.Drain(ctx)
		}
...
func (r *Relay) drainBatch(ctx context.Context) (int, int) {
	...
	if err != nil {
		r.logger.Error().Err(err).Msg("deposit outbox relay: failed to list unpublished events")
		return 0, 0
	}
	...
	for _, evt := range events {
		...
		if pubErr != nil {
			...
			r.logger.Error().Err(pubErr).
				...
				Msg("deposit outbox relay: failed to publish event; will retry")
			return published, len(events)
		}
		...
		if _, markErr := r.outboxRepository.MarkPublished(ctx, evt.ID); markErr != nil {
			...
			r.logger.Error().Err(markErr).
				Str("event_id", evt.EventID).
				Msg("deposit outbox relay: failed to mark event as published; duplicate delivery possible")
			return published, len(events)
		}
