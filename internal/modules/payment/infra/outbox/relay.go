...
// Relay drains the payment module's transactional outbox (payments_outbox) and
// publishes pending events to JetStream. Delivery is at-least-once: an event is
// only marked published after JetStream acknowledges it, and redeliveries are
// deduplicated by the stream's duplicate window keyed on Nats-Msg-Id (the
// domain event ID). Multiple relay instances are safe: MarkPublished is guarded
// by `published_at IS NULL`, so duplicates are bounded and deduplicated anyway.
type Relay struct {
	outboxRepository ports.PaymentOutboxRepository
...
func (relay *Relay) run(ctx context.Context) {
	...
	relay.logger.Info().
		...
		Msg("payment outbox relay started")

	for {
		select {
		case <-ctx.Done():
			relay.logger.Info().Msg("payment outbox relay stopped: context cancelled")
			return
		case <-relay.done:
			relay.logger.Info().Msg("payment outbox relay stopped")
			return
		case <-ticker.C:
			relay.Drain(ctx)
		}
...
func (relay *Relay) drainBatch(ctx context.Context) (int, int) {
	...
	if err != nil {
		relay.logger.Error().Err(err).Msg("payment outbox relay: failed to list unpublished events")
		return 0, 0
	}
	...
	for _, evt := range events {
		...
		if pubErr != nil {
			relay.logger.Error().Err(pubErr).
				...
				Msg("payment outbox relay: failed to publish event; will retry")
			return published, len(events)
		}
		...
		if _, markErr := relay.outboxRepository.MarkPublished(ctx, evt.ID); markErr != nil {
			relay.logger.Error().Err(markErr).
				Str("event_id", evt.EventID).
				Msg("payment outbox relay: failed to mark event as published; duplicate delivery possible")
			return published, len(events)
		}

		published++
		paymentEventsRelayedTotal.Inc()
	}
