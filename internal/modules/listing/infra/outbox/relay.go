...
// Relay drains the listing module's transactional outbox
// (listing_outbox) and publishes pending events to JetStream. Delivery is
...
func (r *Relay) run(ctx context.Context) {
	...
	r.logger.Info().
		...
		Msg("listing outbox relay started")

	for {
		select {
		case <-ctx.Done():
			r.logger.Info().Msg("listing outbox relay stopped: context cancelled")
			return
		case <-r.done:
			r.logger.Info().Msg("listing outbox relay stopped")
			return
		case <-ticker.C:
			r.Drain(ctx)
		}
...
func (r *Relay) drainBatch(ctx context.Context) (int, int) {
	...
	if err != nil {
		r.logger.Error().Err(err).Msg("listing outbox relay: failed to list unpublished events")
		return 0, 0
	}
	...
	for _, evt := range events {
		...
		if pubErr != nil {
			...
			r.logger.Error().Err(pubErr).
				...
				Msg("listing outbox relay: failed to publish event; will retry")
			return published, len(events)
		}
		...
		if _, markErr := r.outboxRepository.MarkPublished(ctx, evt.ID); markErr != nil {
			...
			r.logger.Error().Err(markErr).
				Str("event_id", evt.EventID).
				Msg("listing outbox relay: failed to mark event as published; duplicate delivery possible")
			return published, len(events)
		}
