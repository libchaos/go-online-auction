...
-- Transactional outbox for payment domain events: events are inserted in the
-- same database transaction as the state change that produced them, then
-- relayed to NATS JetStream by the payment module's own outbox relay.
...
CREATE TABLE payments_outbox (
    ...
    CONSTRAINT uq_payments_outbox_event_id UNIQUE (event_id)
);

-- Partial index: the relay only ever scans unpublished rows in insertion order
CREATE INDEX idx_payments_outbox_unpublished ON payments_outbox (id)
