...
-- Trigger to enforce that new bids are higher than current highest bid
...
BEGIN
    -- Get highest bid from auction record (lock the auction row)
    SELECT COALESCE(highest_bid_amount_in_cents, 0) INTO current_highest
    FROM auctions
...
-- +goose Down
DROP TRIGGER IF EXISTS enforce_higher_bids ON bids;
